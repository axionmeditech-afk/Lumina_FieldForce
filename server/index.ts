...
function configureExpoAndLanding(app: express.Application) {
  ...
  // Fallback for SPA routing in web
...
(async () => {
  ...
  const hostCandidates =
    process.platform === "win32" ? ["0.0.0.0", "127.0.0.1"] : ["0.0.0.0"];

  const listenWithFallback = (hostIndex: number) => {
    ...
    const onError = (error: NodeJS.ErrnoException) => {
      const canFallback =
        (error?.code === "ENOTSUP" || error?.code === "EADDRNOTAVAIL") &&
        hostIndex < hostCandidates.length - 1;
      if (canFallback) {
        console.warn(
          `[server] listen failed on ${host}:${port} (${error.code}). Trying ${
            hostCandidates[hostIndex + 1]
          }...`
        );
